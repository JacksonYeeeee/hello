...
class Model(object):
    ...
    def train_model(self,train_X,train_Y=None,test_X=None,test_Y=None,sess=None,summ=None,load_saver=''):
        ...
        if load_saver!='f':
            ...
            print('')
            np.savetxt("./models_DBN2/saver/loss_and_acc.csv", self.loss_and_acc, fmt='%.4f',delimiter=",")
    ...
    def show_and_save_result(self,figname):
        ...
        if self.test_Y is not None:
            ...
            if self.use_for=='classification':
                np.savetxt("./models_DBN2/saver/best_acc.csv", self.best_acc, fmt='%.4f',delimiter=",")
                np.savetxt("./models_DBN2/saver/label_distribution.csv", self.label_distribution, fmt='%.4f',delimiter=",")
                np.savetxt("./models_DBN2/saver/real_class.csv", self.real_class, fmt='%.4f',delimiter=",")
                np.savetxt("./models_DBN2/saver/pred_class.csv", self.pred_class, fmt='%.4f',delimiter=",")          
    ...
    def plot_curve(self,figname):
        ...
        if self.use_for=='classification':
            ...
            ax1.plot(x, self.loss_and_acc[:,0],color='#FFA100',label='loss')
            ...
            ax2.plot(x, self.loss_and_acc[:,1],color='#00AA72',label='train_acc')
            if self.test_Y is not None:
                ax2.plot(x, self.loss_and_acc[:,2],color='#0772A1',label='val_acc')
        ...
        if not os.path.exists('./model_DBN2/saver/img'): os.makedirs('./model_DBN2/saver/img')
        plt.savefig('./model_DBN2/saver/img/'+figname+'.png',bbox_inches='tight')
