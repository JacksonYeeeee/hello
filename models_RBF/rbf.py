...
def bp_train(feature, label, n_hidden, maxCycle, alpha, n_output):
    ...
    while iter <= maxCycle:
        ...
        for j in range(n_hidden):
            ...
            sum3 = 0.0
            for i in range(m):
                sum1 += error[i,:].T * exp(-1.0 * (feature[i]-center[j]) * (feature[i]-center[j]).T / (2 * delta[0,j]*delta[0,j])) * (feature[i] - center[j])
                sum2 += error[i,:].T * exp(-1.0 * (feature[i]-center[j]) * (feature[i]-center[j]).T / (2 * delta[0,j]*delta[0,j])) * (feature[i] - center[j]) * (feature[i] - center[j]).T
                sum3 += error[i,:].T * exp(-1.0 * (feature[i]-center[j]) * (feature[i]-center[j]).T / (2 * delta[0,j]*delta[0,j]))
       ...
       # 2.3、 修正权重和rbf函数中心和扩展常数       
            ...
            w[j,:] = w[j,:] + (alpha * delta_w).reshape(1,-1)
        if iter % 10 == 0:
...
def save_model_result(center, delta, w, result):
    ...
    write_file("./models_RBF/center.txt", center)
    write_file("./models_RBF/delta.txt", delta)
    write_file("./models_RBF/weight.txt", w)
    write_file('./models_RBF/train_result.txt',result)
...
if __name__ == "__main__":
    ...
    feature = reshape(train_data_set,(len(train_data_set),-1))
    label = h5f['Y']
    # 2、训练网络模型
    print ("--------- 2.training ------------")
    ...
    center, delta, w = bp_train(feature, label, 20, 5000, learning_rate, 2)
    # 3、得到最终的预测结果
