import numpy as np
import model_cnn.Activators   # 引入自定义的激活器模块
import math
...
class ConvLayer(object):#(28, 28, 1, 5, 5, 6, 0, 1, Activators.SigmoidActivator(),0.02)   (12, 12, 6, 5, 5, 12, 0, 1, Activators.SigmoidActivator(),0.02)
    ...
    def bp_sensitivity_map(self, sensitivity_array,activator):   # 公式9   卷积层1不需要此步骤，因为卷积层一的误差反向传播出去也没用
        ...
        # print(zp)  #zp为2
        padded_array = padding(expanded_error_array, zp)  #补0操作 64*21*21
        # 初始化delta_array，用于保存传递到上一层的sensitivity map
        self.delta_array = self.create_delta_array()   #64*19*19
        ...
        for i in range(self.filter_number):   #filter_number=12 遍历每一个过滤器。每个过滤器都产生多通道的误差，多个多通道的误差叠加
            ...
            # 将滤波器每个通道的权重权重翻转180度。
            flipped_weights=[]
            for oneweight in filter.get_weights():  #  64*3*3这一个滤波器下的每个通道都进行180翻转  为什么要翻转？？？书上推导出来的
                flipped_weights.append(np.rot90(oneweight, 2))
            flipped_weights = np.array(flipped_weights)
            # 计算与一个filter对应的delta_array
            delta_array = self.create_delta_array()  #  64*19*19
            for d in range(delta_array.shape[0]):   # 计算每个通道上的误差，存储在delta_array的对应通道上
                # print('大小：\n',flipped_weights[d])
                conv(padded_array[i], flipped_weights[d],delta_array[d], 1, 0)#  每计算一个d的循环得到一个64*19*19的三维矩阵
            self.delta_array += delta_array   # 将每个滤波器每个通道产生的误差叠加  最后得到 64*19*19的误差矩阵
    ...
    def bp_gradient(self, sensitivity_array):
        ...
        expanded_error_array = self.expand_sensitivity_map(sensitivity_array)  #64*19*19
        for i in range(self.filter_number):  # 每个过滤器产生一个输出
            # 计算每个权重的梯度
            filter = self.filters[i]
            a=np.zeros((self.channel_number,filter.weights_grad.shape[1],filter.weights_grad.shape[2]))
            for d in range(filter.weights.shape[0]):   # 过滤器的每个通道都要计算梯度
                a[d]=conv(self.padded_input_array[d],expanded_error_array[i],filter.weights_grad[d], 1, 0)   #  公式（31、32中间）
#                                  64*19*19            64*17*17                    64*3*3
            # self.weights_grad 这里的for d in 循环就是循环64次，得到一个64*3*3，在for i in 大循环中，循环64次，得到64个64*3*3
            # 计算偏置项的梯度
